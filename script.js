// ============================================
// KRONOS SYSTEM v14.0 - COMPLETO E CORRIGIDO
...
document.addEventListener('DOMContentLoaded', async () => {
    console.log('🔥 KRONOS SYSTEM v14.0 iniciando...');
    atualizarDataHora();
        ...
        for (const [id, u] of Object.entries(usuarios)) {
            if (u.usuario === 'MattheCarvalho') {
                ...
                if (u.senha !== 'admin123') {
                    ...
                    console.log('🔧 Senha do Super Admin atualizada para: admin123');
                }
                break;
            }
        }
...
function atualizarPaginaAtiva() {
    ...
    for (const [id, funcao] of Object.entries(paginas)) {
...
async function criarConta() {
    ...
    console.log('📝 Tentando criar conta:', usuario);
...
async function fazerLogin() {
    ...
    try {
        // ACESSO DIRETO SUPER ADMIN (sempre funciona)
        if (login === 'MattheCarvalho') {
            const snap = await usuariosRef.once('value');
            const usuarios = snap.val() || {};
            let userId = null;
            ...
            for (const [id, u] of Object.entries(usuarios)) {
                if (u.usuario === 'MattheCarvalho') {
                    ...
                    // Verificar senha
                    if (u.senha !== senha) {
                        // Atualizar senha se estiver errada
                        if (senha === 'admin123') {
                            await usuariosRef.child(id).update({ senha: 'admin123' });
                        } else {
...
function verificarSessaoSalva() {
    const dados = localStorage.getItem('kronos_user');
    if (dados) {
        try {
            window.usuarioLogado = JSON.parse(dados);
            entrarSistema();
        } catch (e) {
...
function renderLinha(id, c) {
    ...
    return `<tr>
        <td><strong>${c.nome}</strong>${tel1}</td><td>${c.marca}</td>
        <td>Dia ${c.dia_vencimento} - ${c.mes_referencia||'-'}</td>
        <td>${formatMoeda(c.valor_total||0)}</td><td>${contarPagas(c)}/${c.qtd_parcelas||0}</td>
            ...
            <div class="modal-body" style="padding:25px;">
                <div style="display:grid;grid-template-columns:1fr 1fr;gap:20px;margin-bottom:25px;">
                    <div style="background:var(--bg-tertiary,#152238);padding:20px;border-radius:14px;">
                        <h4 style="color:var(--gold);">DADOS</h4>
                    ...
                    <div style="background:var(--bg-tertiary,#152238);padding:20px;border-radius:14px;">
                        <h4 style="color:var(--gold);">TELEFONES</h4>
                        <div style="display:grid;gap:12px;">
                ...
                <div style="display:grid;grid-template-columns:1fr 1fr;gap:20px;margin-bottom:25px;">
                    <div style="background:var(--bg-tertiary,#152238);padding:20px;border-radius:14px;">
                        <h4 style="color:var(--gold);">FINANCEIRO</h4>
                        <div style="display:grid;gap:12px;">
                    ...
                    <div style="background:var(--bg-tertiary,#152238);padding:20px;border-radius:14px;">
                        <h4 style="color:var(--gold);">VENCIMENTO</h4>
                        <div style="display:grid;gap:12px;">
                ...
                <div style="background:var(--bg-tertiary,#152238);padding:20px;border-radius:14px;margin-bottom:20px;">
                    <h4 style="color:var(--gold);">PARCELAS (${parcelas.length})</h4>
    ...
    Object.entries(todosClientes).forEach(([id, c]) => {
        ...
        Object.entries(c.parcelas).forEach(([key, p]) => {
            ...
            if (dv.getMonth() === mesAtual && dv.getFullYear() === anoAtual) {
                parcelasMes.push({ clienteNome: c.nome, marca: c.marca, telefone: c.telefone1 || 'N/A', diaVencimento: c.dia_vencimento, ...p });
            }
        });
    });
...
function carregarRelatorioSemanal() {
    ...
    Object.entries(todosClientes).forEach(([id, c]) => {
        ...
        Object.entries(c.parcelas).forEach(([key, p]) => {
            const dv = new Date(p.vencimento);
            if (dv >= seg && dv <= dom) parcelasSemana.push({ clienteNome: c.nome, marca: c.marca, telefone: c.telefone1 || 'N/A', diaVencimento: c.dia_vencimento, ...p });
        });
    });
    ...
    container.innerHTML = `<div>
        <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:20px;">
            <div><h3 style="color:var(--gold);">📊 RELATÓRIO SEMANAL</h3><small style="color:#94a3b8;">${formatData(seg.toISOString())} até ${formatData(dom.toISOString())}</small></div>
            <div style="display:flex;gap:8px;">
                <button class="btn-sm" onclick="carregarRelatorioMensal()">MENSAL</button>
                <button class="btn-sm" onclick="carregarRelatorioSemanal()" style="background:rgba(226,185,75,0.2);color:#e2b94b;">SEMANAL</button>
                <button class="btn-sm" onclick="carregarRelatorioCompleto()">COMPLETO</button>
                <button class="btn-primary" onclick="imprimirRelatorio()"><i class="fas fa-print"></i> IMPRIMIR</button>
            </div>
        </div>
...
function carregarRelatorioCompleto() {
    ...
    Object.entries(todosClientes).forEach(([id, c]) => {
        if (!c.parcelas) return;
        Object.entries(c.parcelas).forEach(([key, p]) => todasParcelas.push({ clienteNome: c.nome, marca: c.marca, telefone: c.telefone1 || 'N/A', diaVencimento: c.dia_vencimento, ...p }));
    });
    ...
    container.innerHTML = `<div>
        <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:20px;">
            <div><h3 style="color:var(--gold);">📊 RELATÓRIO COMPLETO</h3><small style="color:#94a3b8;">Todas as parcelas</small></div>
            <div style="display:flex;gap:8px;">
                <button class="btn-sm" onclick="carregarRelatorioMensal()">MENSAL</button>
                <button class="btn-sm" onclick="carregarRelatorioSemanal()">SEMANAL</button>
                <button class="btn-sm" onclick="carregarRelatorioCompleto()" style="background:rgba(226,185,75,0.2);color:#e2b94b;">COMPLETO</button>
                <button class="btn-primary" onclick="imprimirRelatorio()"><i class="fas fa-print"></i> IMPRIMIR</button>
            </div>
        </div>
        <div style="display:grid;grid-template-columns:repeat(4,1fr);gap:15px;margin-bottom:20px;">
...
console.log('🚀 KRONOS SYSTEM v14.0 - PRONTO!');
