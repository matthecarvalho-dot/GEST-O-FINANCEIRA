// ============================================
// KRONOS SYSTEM v10.0 - COMPLETO
// Logo: kronos_system.jpg
// Dias: 5, 15, 20, 30 + Salvamento Automático
...
document.addEventListener('DOMContentLoaded', async () => {
    console.log('🔥 KRONOS SYSTEM v10.0 iniciando...');
    console.log('📡 Tempo Real ativado para todos os usuários');
...
async function verificarBanco() {
    ...
    if (!dados.clientes || Object.keys(dados.clientes).length === 0) {
        await clientesRef.set({
            ...
            _ex2: {
                nome: "COMÉRCIO BETA", marca: "MARCA PRATA",
                telefone1: "(21) 98888-0002", telefone2: "(21) 2222-0002",
            ...
            _ex3: {
                ...
                parcelas: {
                    ...
                    p3: { numero:3, valor:1000, vencimento:"2023-03-15", status:"quitado", data_pagamento:"2023-03-12" }
                }
            },
            _ex4: {
                nome: "EMPRESA 30 DIAS", marca: "MARCA OURO",
                ...
                parcelas: {
                    ...
                    p3: { numero:3, valor:1000, vencimento:"2024-05-30", status:"pendente", data_pagamento:null }
                }
            }
        });
        console.log('✅ Clientes exemplo criados (Dias 5, 15, 20, 30)');
    }
    
    // TEMPO REAL: Escuta todas as alterações
    carregarClientesTempoReal();
}

// ============================================
// TEMPO REAL - Todos usuários veem alterações
// ============================================
function carregarClientesTempoReal() {
    // Escuta em tempo real qualquer alteração nos clientes
...
function atualizarPaginaAtiva() {
    ...
    const clientesPage = document.getElementById('page-clientes');
    const dia5Page = document.getElementById('page-dia5');
    const dia15Page = document.getElementById('page-dia15');
    const dia20Page = document.getElementById('page-dia20');
    const dia30Page = document.getElementById('page-dia30');
    const parcelasPage = document.getElementById('page-parcelas');
    
    if (dashboardPage && dashboardPage.classList.contains('active')) atualizarDashboard();
    else if (clientesPage && clientesPage.classList.contains('active')) carregarTabelaClientes();
    else if (dia5Page && dia5Page.classList.contains('active')) carregarClientesDia(5);
    else if (dia15Page && dia15Page.classList.contains('active')) carregarClientesDia(15);
    else if (dia20Page && dia20Page.classList.contains('active')) carregarClientesDia(20);
    else if (dia30Page && dia30Page.classList.contains('active')) carregarClientesDia(30);
    else if (parcelasPage && parcelasPage.classList.contains('active')) carregarTodasParcelas();
}
...
function showPage(page) {
    ...
    const titles = {
        ...
        dia15: 'Vencimento Dia 15',
        dia20: 'Vencimento Dia 20',
        dia30: 'Vencimento Dia 30',
...
function criarGraficoStatus(clientes) {
    ...
    charts.status = new Chart(ctx, { type:'doughnut',
...
function criarGraficoComparativo(clientes) {
    ...
    charts.comp = new Chart(ctx, { type:'bar',
        data:{ labels:['DIA 5','DIA 15','DIA 20','DIA 30'],
               datasets:[
                   { label:'Total', data:[d5t,d15t,d20t,d30t], backgroundColor:'#3498db', borderRadius:5 },
                   { label:'Recebido', data:[d5r,d15r,d20r,d30r], backgroundColor:'#27ae60', borderRadius:5 }
               ] },
        options:{ responsive:true, maintainAspectRatio:false,
                  plugins:{ legend:{ labels:{ color:'#8899aa' } } },
                  scales:{ y:{ grid:{ color:'rgba(255,255,255,0.05)' }, ticks:{ color:'#8899aa' } },
...
function renderLinha(id, c) {
    ...
    return `<tr>
        ...
        <td>
            <button class="btn-sm edit" onclick="abrirPerfilCliente('${id}')" title="Perfil"><i class="fas fa-id-card"></i></button>
...
// ============================================
// PERFIL CLIENTE (COM 2 TELEFONES + AUTO SAVE)
// ============================================
    ...
    modal.innerHTML = `
        <div class="modal-modern" style="width:950px;max-width:98%;max-height:90vh;">
            <div class="modal-header" style="background:linear-gradient(135deg,rgba(201,168,76,0.1),transparent);">
                <div><h2>${c.nome}</h2><small style="color:#8899aa;">${c.marca} • Dia ${c.dia_vencimento} • ${c.mes_referencia||'N/A'}</small></div>
            ...
            <div class="modal-body" style="padding:25px;">
                <div style="display:grid;grid-template-columns:1fr 1fr;gap:20px;margin-bottom:25px;">
                    <div style="background:var(--bg-tertiary,#152238);padding:20px;border-radius:12px;">
                        ...
                        <div style="display:grid;gap:12px;">
                            <div><small style="color:#8899aa;">NOME</small><input id="editNome_${id}" class="form-input" value="${c.nome}" style="margin-top:4px;" onchange="salvarCampoAuto('${id}')"></div>
                            <div><small style="color:#8899aa;">MARCA</small><input id="editMarca_${id}" class="form-input" value="${c.marca}" style="margin-top:4px;" onchange="salvarCampoAuto('${id}')"></div>
                        </div>
                    </div>
                    <div style="background:var(--bg-tertiary,#152238);padding:20px;border-radius:12px;">
                        <h4 style="color:var(--gold);margin-bottom:15px;"><i class="fas fa-phone"></i> TELEFONES</h4>
                        <div style="display:grid;gap:12px;">
                            <div><small style="color:#8899aa;">📱 PRINCIPAL</small><input type="tel" id="editTel1_${id}" class="form-input" value="${c.telefone1}" placeholder="(00) 00000-0000" style="margin-top:4px;" onchange="salvarCampoAuto('${id}')"></div>
                            <div><small style="color:#8899aa;">📱 SECUNDÁRIO</small><input type="tel" id="editTel2_${id}" class="form-input" value="${c.telefone2}" placeholder="(00) 00000-0000" style="margin-top:4px;" onchange="salvarCampoAuto('${id}')"></div>
                        </div>
                    </div>
                </div>
                <div style="display:grid;grid-template-columns:1fr 1fr;gap:20px;margin-bottom:25px;">
                    <div style="background:var(--bg-tertiary,#152238);padding:20px;border-radius:12px;">
                        ...
                        <div style="display:grid;gap:12px;">
                            <div><small style="color:#8899aa;">VALOR TOTAL</small><input type="number" id="editValor_${id}" class="form-input" value="${c.valor_total}" step="0.01" style="margin-top:4px;" onchange="recalcParcela('${id}');salvarCampoAuto('${id}')"></div>
                            <div><small style="color:#8899aa;">PARCELAS</small><input type="number" id="editQtd_${id}" class="form-input" value="${c.qtd_parcelas}" min="1" style="margin-top:4px;" onchange="recalcParcela('${id}');salvarCampoAuto('${id}')"></div>
                            <div style="background:rgba(201,168,76,0.1);padding:12px;border-radius:8px;text-align:center;"><small style="color:#8899aa;">VALOR PARCELA</small><strong style="color:var(--gold);font-size:1.2rem;" id="prevValor_${id}">${formatMoeda(c.valor_parcela||(c.valor_total/c.qtd_parcelas))}</strong></div>
                        </div>
                    </div>
                    <div style="background:var(--bg-tertiary,#152238);padding:20px;border-radius:12px;">
                        <h4 style="color:var(--gold);margin-bottom:15px;"><i class="fas fa-calendar"></i> VENCIMENTO</h4>
                        <div style="display:grid;gap:12px;">
                            <div><small style="color:#8899aa;">DIA</small>
                                <select id="editDia_${id}" class="form-input" style="margin-top:4px;" onchange="salvarCampoAuto('${id}')">
                                    <option value="5" ${c.dia_vencimento==5?'selected':''}>DIA 5</option>
                                    <option value="15" ${c.dia_vencimento==15?'selected':''}>DIA 15</option>
                                    <option value="20" ${c.dia_vencimento==20?'selected':''}>DIA 20</option>
                                    <option value="30" ${c.dia_vencimento==30?'selected':''}>DIA 30</option>
                                </select>
                            </div>
                            <div><small style="color:#8899aa;">MÊS</small><input type="month" id="editMes_${id}" class="form-input" value="${c.mes_referencia||''}" style="margin-top:4px;" onchange="salvarCampoAuto('${id}')"></div>
                        </div>
                    </div>
                </div>
                <div style="background:var(--bg-tertiary,#152238);padding:20px;border-radius:12px;margin-bottom:20px;">
                    ...
                    <div style="max-height:300px;overflow-y:auto;">
                        ${parcelas.map(([key,p]) => `
                            <div style="display:flex;justify-content:space-between;align-items:center;padding:12px;background:var(--bg-card);border-radius:8px;margin-bottom:8px;border:1px solid var(--border);">
                                <div style="display:flex;align-items:center;gap:15px;">
                                    ...
                                    <div><strong style="color:#fff;">Parcela ${p.numero}ª</strong><small style="color:#8899aa;display:block;">${formatData(p.vencimento)} • ${formatMoeda(p.valor)}</small></div>
                                </div>
                                <div style="display:flex;align-items:center;gap:10px;">
                ...
                <div style="display:grid;grid-template-columns:repeat(4,1fr);gap:15px;">
                    <div style="background:rgba(39,174,96,0.1);padding:15px;border-radius:10px;text-align:center;"><small style="color:#27ae60;">PAGO</small><strong style="color:#fff;font-size:1.1rem;">${formatMoeda(calcRecebido(c))}</strong></div>
                    <div style="background:rgba(231,76,60,0.1);padding:15px;border-radius:10px;text-align:center;"><small style="color:#e74c3c;">ABERTO</small><strong style="color:#fff;font-size:1.1rem;">${formatMoeda(calcAberto(c))}</strong></div>
                    <div style="background:rgba(52,152,219,0.1);padding:15px;border-radius:10px;text-align:center;"><small style="color:#3498db;">PAGAS</small><strong style="color:#fff;font-size:1.1rem;">${contarPagas(c)}/${c.qtd_parcelas}</strong></div>
                    <div style="background:rgba(201,168,76,0.1);padding:15px;border-radius:10px;text-align:center;"><small style="color:#c9a84c;">STATUS</small><strong style="color:#fff;font-size:1.1rem;">${st.toUpperCase()}</strong></div>
                </div>
            </div>
            <div class="modal-footer">
                <small id="msgSalvo_${id}" style="color:#27ae60;margin-right:auto;">✅ Dados salvos automaticamente</small>
...
async function salvarCliente() {
    ...
    for (let i = 1; i <= qp; i++) {
        let diaVenc = dv;
        // Ajustar dia 30 para meses com menos dias
        if (dv === 30) {
    ...
    const dados = {
        nome, marca, telefone1: '', telefone2: '',
...
async function carregarUsuarios() {
    ...
    if (window.usuarioLogado) {
        document.getElementById('meuPerfil').innerHTML = `<h4>${window.usuarioLogado.nome}</h4><p>${window.usuarioLogado.cargo} • ${window.usuarioLogado.nivel.replace('_',' ').toUpperCase()}</p><small style="color:#8899aa;">@${window.usuarioLogado.usuario}</small>`;
    }
    const container = document.getElementById('listaUsuarios');
    const lista = Object.entries(usuarios);
    if (lista.length === 0) { container.innerHTML = '<p style="color:#8899aa;text-align:center;padding:20px;">Nenhum</p>'; return; }
    container.innerHTML = lista.map(([id, u]) => {
        const isMe = window.usuarioLogado && window.usuarioLogado.id === id;
        return `<div style="display:flex;justify-content:space-between;align-items:center;padding:14px 0;border-bottom:1px solid #1e293b;"><div style="display:flex;align-items:center;gap:12px;"><div style="width:40px;height:40px;border-radius:50%;background:rgba(212,168,83,0.15);display:flex;align-items:center;justify-content:center;color:#c9a84c;font-weight:700;">${u.nome.charAt(0)}</div><div><strong style="color:#fff;">${u.nome} ${isMe?'(Você)':''}</strong><small style="color:#8899aa;display:block;">@${u.usuario} • ${u.cargo}</small></div></div><div style="display:flex;gap:6px;"><button class="btn-sm edit" onclick="editarUsuario('${id}')"><i class="fas fa-edit"></i></button>${!isMe?`<button class="btn-sm" onclick="toggleUsuario('${id}',${!u.ativo})"><i class="fas fa-power-off"></i></button><button class="btn-sm danger" onclick="excluirUsuario('${id}')"><i class="fas fa-trash"></i></button>`:''}</div></div>`;
    }).join('');
}
...
console.log('🚀 KRONOS SYSTEM v10.0 - PRONTO!');
